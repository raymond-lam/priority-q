...
The priority queue's iterator successively dequeues elements from a shallow clone of the priority queue, so that one could, for example, iterate over the elements of the priority queue in sorted order with a `for ... of` loop, and the priority queue itself will not be mutated.
